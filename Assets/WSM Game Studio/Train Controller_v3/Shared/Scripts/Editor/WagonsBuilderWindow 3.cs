...
namespace WSMGameStudio.RailroadSystem
{
    public class WagonsBuilderWindow : EditorWindow
    {
        ...
        /// </summary>
        [MenuItem("WSM Game Studio/Train Controller/Utilities/Custom Wagons Creator")]
        [MenuItem("Window/WSM Game Studio/Custom Wagons Creator")]
        public static void ShowWindow()
        {
            WagonsBuilderWindow currentWindow = GetWindow<WagonsBuilderWindow>("Custom Wagons Creator");
            currentWindow.minSize = new Vector2(330, 120);
        }
        ...
        private void CreateWagon()
        {
            ...
            if (!valid)
            {
                ShowMessage(_txtMessage, Splines.MessageType.Error);
                return;
            }

            bool success = CustomWagonCreator.Create(_customProfile, _modelPrefab, _wagonName, out _txtMessage);
            if (success)
                ShowMessage(_txtMessage, Splines.MessageType.Success);
        }
